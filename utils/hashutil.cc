...
namespace fawn {
	...
	*idx1=c; *idx2=b;
    ...
    void HashUtil::BobHash(const string &s, uint32_t *idx1,  uint32_t *idx2)
    {
    ...
    /*
     * 4-byte integer hash, full avalanche with 6 shifts and magic constatnts
     */
    uint32_t HashUtil::hashint_full_avalanche_1( uint32_t a)
    {
        a = (a+0x7ed55d16) + (a<<12);
        a = (a^0xc761c23c) ^ (a>>19);
        a = (a+0x165667b1) + (a<<5);
    ...
    /*
     * 4-byte integer hash, full avalanche with 7 shifts and NO magic constatnts
     */
    uint32_t HashUtil::hashint_full_avalanche_2( uint32_t a)
    {
        a -= (a<<6);
        a ^= (a>>17);
        a -= (a<<9);
    ...
    uint32_t HashUtil::hashint_half_avalanche( uint32_t a)
    {
        a = (a+0x479ab41d) + (a<<8);
        a = (a^0xe4aa10ce) ^ (a>>5);
        a = (a+0x9942f0a6) - (a<<14);
...
#define FNV_32_PRIME ((uint32_t)0x01000193)
     ...
     */
    uint32_t HashUtil::FNVHash(const void *buf, size_t len)
    {
    ...
    uint32_t HashUtil::FNVHash(const string &s)
    {
	return FNVHash(s.data(), s.length());
    }
    ...
    uint32_t HashUtil::MurmurHash(const string &s, uint32_t seed)
    {
    ...
    uint32_t HashUtil::SuperFastHash(const string &s)
    {
